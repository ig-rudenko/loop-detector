...
from app.schemas.auth import LoginSchema, RefreshSchema, TokenSchema, UserSchema, VerifySchema
...
async def get_token_api_view(user_data: LoginSchema):
    try:
        ...
        raise HTTPException(status_code=exc.status or 500, detail=exc.message) from exc
...
async def get_user_api_view(user: Annotated[UserSchema, Depends(get_current_user)]):
    return user
